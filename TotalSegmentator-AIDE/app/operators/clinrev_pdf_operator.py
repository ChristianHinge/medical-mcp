# Clinical Review PDF Encapsulated DICOM generator
#
# This operator creates a PDF Encapsulated DICOM containing summary information about the input DICOM dataset and the
# CT imaging data with corresponding segmentations generated by TotalSegmentator.
#
...
class ClinicalReviewPDFGenerator(Operator):
    ...
    def generate_report_pdf(ds_meta: pydicom.Dataset,
        ...
        gstt_csc = Table([["Packaged for AIDE by:", csc_logo, gstt_logo]], style=chart_style_footer)
        uhb = Table([["Created by:", uhb_logo]], style=chart_style_footer)
        story.append(Table([[gstt_csc, uhb]], style=chart_style_footer))
        ...
        story.append(Paragraph("TotalSegmentator performs AI-driven automated segmentation of 104 regions in CT "
                               ...
                               "TotalSegmentator: www.github.com/GSTT-CSC/TotalSegmentator-AIDE", styleN))
        ...
        patient_image_width = 8 * cm
        # Add  images
        ax_im = create_image_flowable(ax_img_path, patient_image_width)
        cor_im = create_image_flowable(cor_img_path, patient_image_width)
        sag_im = create_image_flowable(sag_img_path, patient_image_width)

        chart_style = TableStyle([('ALIGN', (0, 0), (-1, -1), 'CENTER'),
                                  ('VALIGN', (0, 0), (-1, -1), 'MIDDLE')])
        story.append(ax_im)
        story.append(Table([[cor_im, sag_im]], style=chart_style))
...
def create_image_flowable(im_path, width):
    """

    Maintains aspect ratio, sizes for width
    :return: Image flowable for platypus.
    """

    temp = utils.ImageReader(im_path)
    iw, ih = temp.getSize()
    aspect = ih / float(iw)
    img = pl.Image(im_path, width=width, height=(width * aspect))
    img.hAlign = 'CENTER'
    return img
